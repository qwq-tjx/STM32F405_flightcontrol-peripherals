...
void micolink_decode(uint8_t data)
{
    ...
    switch(msg.msg_id)
    {
        case MICOLINK_MSG_ID_RANGE_SENSOR:
        {
            ...
            /* ---- 存储原始数据到全局结构体 (UART3 ISR 原子写入) ---- */
            g_optflow.flow_vel_x  = payload.flow_vel_x;     /* cm/s @ 1m */
            g_optflow.flow_vel_y  = payload.flow_vel_y;     /* cm/s @ 1m */
            g_optflow.distance    = payload.distance;       /* mm, 0=不可用 */
            g_optflow.flow_quality = payload.flow_quality;  /* 0~100 */
            g_optflow.fresh       = 1;                      /* 通知 TIM7 ISR 有新数据 */
            break;
        } 
